...
/* this function receives the current priority queue and a new integer value, then inserts the new integer into queue in descending order. It returns an error code: 0 if the insertion was successful, -1 if the insertion failed*/
...
/* this function receives the current priority queue and an integer pointer. It removes the minimum integer from the queue, stores the removed value into the integer pointer, then returns an error code: 0 if the deletion was successful, -1 if the deletion failed. This function has O(1) time complexity.*/
